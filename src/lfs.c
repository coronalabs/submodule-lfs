...
/* Created by Corona Labs to work-around strerror() compiler warnings. Visual Studio threatens it'll be deprecated. */
static const char* lfs_get_win32_strerror(int errorNumber)
{
	static char sUtf8ErrorMessage[128];
	sUtf8ErrorMessage[0] = '\0';
	strerror_s(sUtf8ErrorMessage, sizeof(sUtf8ErrorMessage), errorNumber);
	return sUtf8ErrorMessage;
}
#define strerror(errorNumber) lfs_get_win32_strerror(errorNumber)
#endif
...
static int change_dir (lua_State *L) {
	...
	/* Added by Corona Labs to compile on WinRT, which does not support any current directory APIs. */
	if (1) {
#elif defined(_WIN32) || defined(WINAPI_FAMILY)
	/* Added by Corona Labs to support Windows unicode paths. */
	wchar_t *utf16Path = lua_create_utf16_string_from(path);
	int result = _wchdir(utf16Path);
...
static int get_dir(lua_State *L) {
	...
	/* Added by Corona Labs to support Windows unicode paths. */
	char utf8Path[1024];
	wchar_t utf16Path[1024];
	utf8Path[0] = '\0';
	utf16Path[0] = L'\0';
	if (_wgetcwd(utf16Path, sizeof(utf16Path) / sizeof(wchar_t)))
	{
		lua_convert_utf16_to_utf8(utf16Path, utf8Path, sizeof(utf8Path));
	}
	if ('\0' == utf8Path[0])
	{
		lua_pushnil(L);
...
static int lfs_lock_dir(lua_State *L) {
  ...
  size_t lnSizeInBytes = pathl + strlen(lockfile) + 1;
  ln = (char*)malloc(lnSizeInBytes);
  if(!ln) { 
    lua_pushnil(L); lua_pushstring(L, strerror(errno)); return 2;
  }
  strcpy_s(ln, lnSizeInBytes, path); strcat_s(ln, lnSizeInBytes, lockfile);
  {
	  /* Modified by Corona Labs to support unicode paths on Windows. */
	  wchar_t *utf16Path = lua_create_utf16_string_from(ln);
	  fd = CreateFileW(
				utf16Path, GENERIC_WRITE, 0, NULL, CREATE_NEW,
...
#if defined(_WIN32) || defined(WINAPI_FAMILY)
	/* Added by Corona Labs to support unicode paths on Windows. */
	wchar_t *utf16Path = lua_create_utf16_string_from(path);
	fail = _wrmdir(utf16Path);
	lua_destroy_utf16_string(utf16Path);
#else
	fail = rmdir(path);
#endif
...
static int dir_iter (lua_State *L) {
	...
	if (d->hFile == 0L) { /* first entry */
		wchar_t *utf16Pattern = lua_create_utf16_string_from(d->pattern);
		d->hFile = _wfindfirst(utf16Pattern, &c_file);
		lua_destroy_utf16_string(utf16Pattern);
		if (-1L == d->hFile) {
			lua_pushnil (L);
			lua_pushstring (L, strerror (errno));
			d->closed = 1;
			...
			char *utf8Path = lua_create_utf8_string_from(c_file.name);
			lua_pushstring(L, utf8Path);
			lua_destroy_utf8_string(utf8Path);
			return 1;
		}
	} else { /* next entry */
...
static int link_info (lua_State *L) {
	if( lua_type(L, 2) == LUA_TSTRING && strcmp(lua_tostring(L, 2), "target") == 0) {
#if defined(WIN32) || defined(EMSCRIPTEN)
#else
		const char *link = luaL_checkstring(L, 1);
		char out[LFS_MAXPATHLEN] = {0};
		if(readlink(link, out, LFS_MAXPATHLEN) > 0) {
